...
PitchShifter::PitchShifter( float shiftAmount, long osampAmount )
{
    ...
    freqPerBin    = audio_engine::SAMPLE_RATE / ( float ) fftFrameSize;
...
{
    // TODO: clear memsets!
    // tough in the current implementation this is never destructed
}
...
*/
void PitchShifter::process( float* sampleBuffer, int bufferLength )
//void smbPitchShift(float pitchShift, long numSampsToProcess, long fftFrameSize, long osamp, float sampleRate, float *indata, float *outdata)
{
    ...
    for ( i = 0; i < bufferLength; ++i )
    {
        ...
        gInFIFO[ gRover ] = sampleBuffer[ i ];
        sampleBuffer[ i ] = gOutFIFO[ gRover - inFifoLatency ];
        gRover++;
